...
function jobposting_install301() {
	...
	$oDatabase->query("
	CREATE TABLE IF NOT EXISTS `". Phpfox::getT('jobposting_category') ."` (
	...
	$oDatabase->query("
	CREATE TABLE IF NOT EXISTS `". Phpfox::getT('jobposting_category_data') ."` (
			`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
			`category_id` int(10) unsigned NOT NULL ,
			`jobposting_id` int(10) unsigned NOT NULL ,
			PRIMARY KEY (`id`),
			KEY `campaign_category` (`category_id`,`jobposting_id`),
			KEY `category_id` (`category_id`)
		)  AUTO_INCREMENT=1 ;
	");

	 $oDatabase->query("INSERT IGNORE INTO `". Phpfox::getT('jobposting_category') ."` (`category_id`, `name`, `parent_id`, `time_stamp`, `used`, `is_active`) VALUES
