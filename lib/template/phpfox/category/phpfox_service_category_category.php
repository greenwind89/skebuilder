...
class [skebuilder:service_class_name] extends Phpfox_Service 
{
	...
	{		
		$sCacheId = $this->cache()->set('[skebuilder:module_name]_category_browse' . ($iCategoryId === null ? '' : '_' . $iCategoryId));
		...
		{					
			...
			{
				$aCategories[$iKey]['url'] = Phpfox::permalink('[skebuilder:module_name].category', $aCategory['category_id'], $aCategory['name']);
				...
				//if ($sCategory === null)
				{
					...
					{
						$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('[skebuilder:module_name].category', $aSubCategory['category_id'], $aSubCategory['name']);
					}
				}
			}
	...
	public function get()
	{
		$sCacheId = $this->cache()->set('[skebuilder:module_name]_category_display_' . $this->_sDisplay . '_' . Phpfox::getLib('locale')->getLangId());
	...
	{		
		$sCacheId = $this->cache()->set('[skebuilder:module_name]_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		if ($aCategories === null)
		{
			$aCategories = $this->database()->select('pc.parent_id, pc.category_id, pc.name')
				->from(Phpfox::getT('[skebuilder:module_name]_category_data'), 'pcd')
				->join($this->_sTable, 'pc', 'pc.category_id = pcd.category_id')
				->where('pcd.[skebuilder:item_name]_id = ' . (int) $iId)
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('[skebuilder:module_name].category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('[skebuilder:module_name].category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}
	...
	public function getCategoryIds($iId)
	{
		$aCategories = $this->database()->select('category_id')
			->from(Phpfox::getT('[skebuilder:module_name]_category_data'))
			->where('[skebuilder:item_name] = ' . (int) $iId)
			->execute('getSlaveRows');
	...
	{
		$sCacheId = $this->cache()->set('[skebuilder:module_name]_category_childern_' . $sCategory);
	...
	{
		$sCacheId = $this->cache()->set('[skebuilder:module_name]_category_parent_' . $sCategory);
	...
	public function __call($sMethod, $aArguments)
	{
		 ...
		 */
		if ($sPlugin = Phpfox_Plugin::get('[skebuilder:module_name].service_category_category__call'))
		{
			return eval($sPlugin);
		}
	...
	private function _get($iParentId, $iActive = null)
	{
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('[skebuilder:module_name].select') : Phpfox::getPhrase('[skebuilder:module_name].select_a_sub_category')) . ':</option>' . "\n";
			}
	...
	private function _getParentsUrl($iParentId, $bPassName = false)
	{
		...
		if ($iCnt === 1)
		{
			...
			$sCacheId = $this->cache()->set('[skebuilder:module_name]_category_url' . ($bPassName ? '_name' : '') . '_' . $iParentId);
